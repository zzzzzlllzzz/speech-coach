...
言镜 AI 是一个面向高中 AI 应用比赛展示的 Web 应用。用户可上传最长 30 分钟的演讲视频，系统会从语言、语音、动作、手势、姿态和镜头交流等角度生成一份可视化训练报告。
...
- 结果可信：真实分析失败时明确停止评分并说明原因；演示数据只通过用户主动选择的示例流程或 `USE_MOCK=true` 使用。
...
- 开始前选择使用场景、训练重点和目标时长，避免拿到与目标无关的通用报告。
- 报告自动聚焦一个最值得先练的维度，并提供 5 分钟计时器、三步清单和可直接照说的提示句。
- 同场景再次录制后，逐维展示与上一次相比的变化，而不只展示孤立总分。
- 分析前显示视频时长、大小和处理策略；长视频仍覆盖完整音频，并在全时长均匀抽取视觉帧。

### 分析可信度

- 真实语音识别失败时直接停止评分并说明原因，不再以演示数据伪装成成功报告。
- 完整音轨检测静音占比、低音量片段、音量稳定性和超过 1.5 秒的停顿时间点。
- 可信度卡同时校验语音文字、视觉抽帧、视觉时长覆盖和完整音轨覆盖。
...
- 生成问题时间点、个性化建议和总结。
- 一键生成两套演讲稿：可提交的纯净改写稿，以及逐句标注停顿、重音、语速、声音、手势、目光和站位的排练导演稿。
- 配置 DeepSeek 后会结合全文语义深度改写；密钥缺失或服务临时异常时自动生成规则版导演稿，不中断训练流程。
- 支持打印或导出报告，当前使用浏览器打印 `window.print()` 实现。
...
项目也安装了 `imageio-ffmpeg` 作为备用方案。如果系统 FFmpeg 不可用，程序会尝试使用备用 FFmpeg；如果仍失败，系统会停止本次评分并提示检查环境，不会返回模拟报告。
...
本地音频按阿里云实时流协议分块发送，默认约 2 倍实时速度，优先避免长音频因发送过快而丢失分句回调。可用 `ALIYUN_NLS_CHUNK_MS` 和 `ALIYUN_NLS_SEND_INTERVAL` 调整，但不建议为了缩短等待而把间隔设得过低。
...
如果模型下载失败、音频中没有清晰人声或运行出错，系统会停止本次评分并显示真实原因，不会用演示稿冒充真实转写。
...
在报告页点击“生成可排练导演稿”，系统会同时给出不带舞台标记的纯净改写稿，以及按句拆分的导演稿。导演稿会标注每句的停顿秒数、语速、重音、声音、手势、目光和站位，可直接复制排练。DeepSeek 不可用时会自动退回内置规则版，按钮不会因缺少密钥而失败。
...
如果浏览器端 MediaPipe 失败，后端会使用 OpenCV 对真实视频帧做近似分析，估算人脸可见比例、近似镜头交流、低头次数、画面晃动、手势活跃度、手部可见比例、遮脸次数和表情变化程度。服务端不再安装 Python MediaPipe，从而避免其 OpenGL/Protobuf 依赖冲突并减小部署镜像。如果 OpenCV 也无法读取视频，相关视觉分数会留空并标记数据不足。
...
- 语音表现：根据语速、口头禅、低音量片段、音量稳定性和过长停顿评分。
...
3. MediaPipe 没检测到人  
   可能与光线、角度、距离或遮挡有关。系统会尝试用后端 OpenCV 分析真实帧；仍无法检测时会明确标记视觉数据不足。

4. FFmpeg 不可用  
   安装系统 FFmpeg，或依赖 `imageio-ffmpeg` 备用方案。两者都失败时系统会停止评分并报告环境问题。
